...
class Factor:
    ...
    def compute_messages(self, eta_damping):
        N_adj = len(self.adj_vIDs)  # 邻接变量个数
        dim_list = [v.dofs for v in self.adj_var_nodes]
        total_dim = sum(dim_list)

        # 1) 构造 "pot" = 因子自身 + 所有 v→f 消息
        pot_eta = self.factor.eta.copy()
        pot_Lam = self.factor.lam.copy()
        offset = 0
        for n in range(N_adj):
            d = dim_list[n]
            pot_eta[offset:offset+d] += (self.adj_beliefs[n].eta - self.messages[n].eta)
            pot_Lam[offset:offset+d, offset:offset+d] += (self.adj_beliefs[n].lam - self.messages[n].lam)
            offset += d
        ...
        for n in range(N_adj):
            ...
            # 2) pot_m_1 = pot - 自己的 v→f 消息
            pot_m_1_eta = pot_eta.copy()
            pot_m_1_Lam = pot_Lam.copy()
            pot_m_1_eta[offset:offset+d] -= (self.adj_beliefs[n].eta - self.messages[n].eta)
            pot_m_1_Lam[offset:offset+d, offset:offset+d] -= (self.adj_beliefs[n].lam - self.messages[n].lam)

            # 3) 边缘化掉除了第 n 个变量以外的其他维度
            aa = slice(offset, offset+d)
            bb = np.concatenate([np.arange(0, offset), np.arange(offset+d, total_dim)])
            aa_eta = pot_m_1_eta[aa]
            aa_Lam = pot_m_1_Lam[aa, aa]

            if bb.size > 0:
                bb_eta = pot_m_1_eta[bb]
                bb_Lam = pot_m_1_Lam[bb[:, None], bb]   # 等价于 np.ix_(bb, bb)
                ab_Lam = pot_m_1_Lam[aa, bb]

                bb_Cov = np.linalg.inv(bb_Lam)
                eta = aa_eta - ab_Lam @ bb_Cov @ bb_eta
                Lam = aa_Lam - ab_Lam @ bb_Cov @ ab_Lam.T
            else:
                eta, Lam = aa_eta, aa_Lam

            # 4) 阻尼更新
            new_eta = (1 - eta_damping) * eta + eta_damping * self.messages[n].eta
            new_lam = (1 - eta_damping) * Lam + eta_damping * self.messages[n].lam

            self.messages[n].eta = new_eta
            self.messages[n].lam = new_lam
            new_msgs.append((new_eta, new_lam))
