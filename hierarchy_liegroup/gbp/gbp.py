...
class FactorGraph:
    ...
    def compute_all_messages(self, factors=None, level=None, local_relin=False):
        if factors is None:
    ...
    def relinearise_factors(self, factors=None):
        ...
        for factor in factors:
            ...
            for var in factor.adj_var_nodes:
                adj_belief_means.append(var.linpoint)
    ...
    def synchronous_iteration(self, factors=None, level=None, local_relin=False, robustify=False):
        if level is not None:
            vars = self.multigrid_vars[level]
            factors = self.multigrid_factors[level]
        else:
    ...
    def compute_factor(self, linpoint=None, update_self=True):
        ...
        if linpoint is None:
            ...
            for var in self.adj_var_nodes:
                self.linpoint += list(var.linpoint)
        else:
        ...
        def se2_from_xytheta(x, y, th):
            c, s = np.cos(th), np.sin(th)
            T = np.eye(3, dtype=float)
            T[0:2, 0:2] = np.array([[c, -s],
                                    [s,  c]], dtype=float)
            T[0:2, 2] = np.array([x, y], dtype=float)
            return T
        ...
        def pose2_localcoords_gtsam_style(T):
            """
            GTSAM Pose2-style 'localCoordinates' for a Pose2 element T:
            returns [tx, ty, theta] where [tx,ty] is the translation part directly.
            """
            R = T[0:2, 0:2]
            t = T[0:2, 2]
            th = wrap_angle(np.arctan2(R[1, 0], R[0, 0]))
        ...
        if isinstance(self.measurement, float):
            ...
            eta_factor = self.measurement_lambda * J.T * (self.measurement - pred_measurement)
        else:
            ...
            if self.linpoint.shape[0] < 4:
                Xi = se2_from_xytheta(*self.linpoint[:3])
                Z  = se2_from_xytheta(*self.measurement)
                E = se2_inv(Z) @ (Xi)  # Z^{-1} Xi^{-1} Xj
                r0 = pose2_localcoords_gtsam_style(E) # [tx, ty, theta]
            else:
                Xi = se2_from_xytheta(*self.linpoint[:3])
                Xj = se2_from_xytheta(*self.linpoint[-3:])
                Z  = se2_from_xytheta(*self.measurement)
                E = se2_inv(Z) @ (se2_inv(Xi) @ Xj)  # Z^{-1} Xi^{-1} Xj
                r0 = pose2_localcoords_gtsam_style(E) # [tx, ty, theta]

            eta_factor = -(J.T @ self.measurement_lambda) @ r0
