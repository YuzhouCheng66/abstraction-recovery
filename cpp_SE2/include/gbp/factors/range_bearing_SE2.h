...
struct RangeBearingSE2 : public Factor {
  std::pair<double, manif::SO2d> measurement_;
  Eigen::Matrix<double, 2, 2> measurement_Lam_;

  RangeBearingSE2(Key key, const std::vector<Key> &neighbors,
                  const std::pair<double, manif::SO2d> &measurement,
                  const Eigen::Matrix<double, 2, 2> &measurement_Lam)
      : Factor(key, neighbors, false, FactorType::RANGEBEARING2D), measurement_(measurement),
  ...
  bool update_factor(const std::map<Key, std::any> &X0, int32_t lin_freq) override {
    ...
    auto [h, J] =
        residual_range_bearing(this->X0_vec_.at(0), this->X0_vec_.at(1), this->measurement_);
