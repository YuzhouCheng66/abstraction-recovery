...
struct BetweenSE2 : public Factor {
  manif::SE2d measurement_;
  Eigen::Matrix<double, 3, 3> measurement_Lam_;

  BetweenSE2(Key key, const std::vector<Key> &neighbors, const manif::SE2d &measurement,
             const Eigen::Matrix<double, 3, 3> &measurement_Lam)
      : Factor(key, neighbors, false, FactorType::BETWEEN2D), measurement_(measurement),
  ...
  bool update_factor(const std::map<Key, std::any> &X0, int32_t lin_freq) override {
    ...
    auto [h, J] = residual_between(this->X0_vec_.at(0), this->X0_vec_.at(1), this->measurement_);
